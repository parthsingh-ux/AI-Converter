...
- MANDATORY IMAGE DIMENSIONS HARD RULE:
  • Image dimensions, image position, image aspect ratio, object-fit, object-position, container dimensions, and responsive image behavior must never be guessed by the AI.
  • These values must come from the deterministic browser/layout analysis layer.
  • The AI may interpret how those values should map to Elementor, but it must not invent or approximate the measurements.
- ZERO HARDCODING & DYNAMIC PRESERVATION DIRECTIVE:
  • NOTHING IS HARDCODED: Extract ALL colors, font families, font sizes, line heights, spacing, borders, tables, and form fields dynamically from the source website.
  • TABLES PRESERVED: Convert source <table>, <thead>, <tbody>, <tr>, <th>, <td> or CSS grid tables into structured Elementor tables or HTML table widgets with exact source cell padding, background, borders, and row colors.
  • FORMS PRESERVED: Convert source form inputs (<input>, <textarea>, <select>, <checkbox>, <radio>) into Gravity Forms widgets or form nodes preserving exact labels, placeholders, options, and submission buttons.
  • FONTS PRESERVED: Preserve exact font families dynamically extracted from the source stylesheet. Never replace source fonts with generic default fonts.
