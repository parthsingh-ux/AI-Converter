...
export function validateTemplate(template) {
  ...
  // Handle case where user passes full object { header_template, footer_template, content_template, content, elements }
  if (typeof template === "object" && !Array.isArray(template)) {
    let checkedAny = false;
    const parts = ["header_template", "footer_template", "content_template", "content", "elements"];

    for (const partKey of parts) {
      if (template[partKey] && Array.isArray(template[partKey])) {
        checkedAny = true;
    ...
    if (!checkedAny) {
      // Treat object itself as a single element node if it has elType or id
      if (template.elType || template.id) {
