...
/**
 * Normalizes any CSS color string (including modern color(srgb r g b / a) and HSL)
 * into standard WordPress Elementor PHP-compatible #HEX or rgba(...) formats.
 */
function normalizeColorValue(colorStr) {
  if (!colorStr || typeof colorStr !== "string") return colorStr;
  let str = colorStr.trim();

  // If inherit, initial, unset, or empty, return null to trigger contrast fallback
  if (/^(inherit|initial|unset|none|""|'')$/i.test(str)) {
    return null;
  }

  // Handle color(srgb r g b / a) or color(srgb r g b)
  if (str.includes("srgb")) {
    const hasAlpha = str.includes("/");
    const matches = str.match(/[\d\.]+/g);
    if (matches && matches.length >= 3) {
      let r = Math.round(parseFloat(matches[0]) <= 1 ? parseFloat(matches[0]) * 255 : parseFloat(matches[0]));
      let g = Math.round(parseFloat(matches[1]) <= 1 ? parseFloat(matches[1]) * 255 : parseFloat(matches[1]));
      let b = Math.round(parseFloat(matches[2]) <= 1 ? parseFloat(matches[2]) * 255 : parseFloat(matches[2]));
      let a = hasAlpha && matches[3] !== undefined ? parseFloat(matches[3]) : 1;
  ...
  // Handle 6-digit hex (#ffffff -> #FFFFFF)
...
function sanitizeNodeOrArray(node, depth, seenIds, parentBgColor, summaryColors = []) {
  if (!node) return null;

  if (Array.isArray(node)) {
    return node
      .map((child) => sanitizeSingleNode(child, depth, seenIds, parentBgColor, summaryColors))
      .filter(Boolean);
  }

  return sanitizeSingleNode(node, depth, seenIds, parentBgColor, summaryColors);
}

function sanitizeSingleNode(node, depth = 0, seenIds = new Set(), parentBgColor = null, summaryColors = []) {
  ...
  // 3b. Normalize All Color Property Keys to standard #HEX or rgba(...)
  const colorPropKeys = [
    "background_color",
    "background_color_b",
    "border_color",
  ...
  for (const key of colorPropKeys) {
    if (key in settings) {
      const normalized = normalizeColorValue(settings[key]);
      if (normalized) {
  ...
  if (copy.elType === "container") {
    ...
    // Ensure container border_color is populated whenever border_width is set
    const isDarkBgContainer = isDarkColor(currentBgColor);
    if (settings.border_width && !settings.border_color) {
      settings.border_color = isDarkBgContainer ? "rgba(255,255,255,0.18)" : "#DBD3C6";
      if (!settings.border_border) settings.border_border = "solid";
    }
  ...
  if (copy.elType === "widget") {
    ...
    // Clean font family string (e.g. '"Inter", sans-serif' -> 'Inter')
    if (settings.typography_font_family) {
      let fFam = String(settings.typography_font_family).split(",")[0].replace(/['"]/g, "").trim();
      if (/^(inherit|initial|unset)$/i.test(fFam)) {
        delete settings.typography_font_family;
      } else {
        settings.typography_font_family = fFam;
      }
    }
    ...
    // Dynamic color resolution from extracted source palette
    const colorsList = Array.isArray(summaryColors) ? summaryColors : [];
    const dynamicDarkColor = (colorsList.find((c) => isDarkColor(c)) || "#2B2723").toUpperCase();
    const dynamicTextColor = (colorsList.find((c) => !isDarkColor(c) && c !== "#FFFFFF") || "#46525E").toUpperCase();

    const defaultTitleColor = isDarkBg ? "#FFFFFF" : dynamicDarkColor;
    const defaultTextColor = isDarkBg ? "#E6EAEF" : dynamicTextColor;
    ...
    if (wType === "heading") {
      ...
      if (!settings.title_color) {
        settings.title_color = settings.color || settings.text_color || settings.heading_color || defaultTitleColor;
      }
      ...
      // Default typography fallbacks for headings
      if (!settings.typography_font_size) {
        const tag = (settings.header_size || settings.tag || "h2").toLowerCase();
        const sizeMap = { h1: 48, h2: 36, h3: 28, h4: 22, h5: 18, h6: 16 };
        const sz = sizeMap[tag] || 32;
        settings.typography_font_size = { unit: "px", size: sz };
      }
      if (!settings.typography_font_weight) {
      ...
      if (!settings.text_color) {
        settings.text_color = settings.color || settings.title_color || settings.body_color || defaultTextColor;
      }
      ...
      if (!settings.typography_font_size) {
        settings.typography_font_size = { unit: "px", size: 16 };
      }
      if (!settings.typography_line_height) {
        settings.typography_line_height = { unit: "em", size: 1.6 };
      }
      settings.typography_typography = "custom";
      ...
      if (!settings.button_text_color) {
        settings.button_text_color = settings.text_color || settings.color || (isDarkBg ? "#FFFFFF" : dynamicDarkColor);
      }
      ...
      if (!settings.title_color) {
        settings.title_color = isDarkBg ? "#FFFFFF" : "#2B2723";
      }
      if (!settings.description_color) {
        settings.description_color = isDarkBg ? "#E6EAEF" : "#46525E";
      }
    } else if (wType === "image") {
      ...
      // Object fit & position preservation
      if (settings["object-fit"] && !settings.object_fit) {
        settings.object_fit = settings["object-fit"];
      }
      if (!settings.object_fit) settings.object_fit = "cover";

      if (settings["object-position"] && !settings.object_position) {
        settings.object_position = settings["object-position"];
      }
      if (!settings.object_position) settings.object_position = "center center";

      // Dimension normalization
      if (settings.width && typeof settings.width === "number") {
        const unit = settings.width_unit || "px";
        settings.width = { unit, size: settings.width };
      }
      if (settings.height && typeof settings.height === "number") {
        const unit = settings.height_unit || "px";
        settings.height = { unit, size: settings.height };
      }
